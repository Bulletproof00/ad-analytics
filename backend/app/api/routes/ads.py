...
from app.db.models import Ad, Advertiser, Score, Tag, Feature
...
def list_ads(
    ...
    if creative_type:
        query = query.join(Feature, isouter=True).where(Feature.creative_type == creative_type)
    if pronoun_style:
        query = query.join(Feature, isouter=True).where(Feature.copy_metrics["pronoun_style"].astext == pronoun_style)
    if hook_type:
        query = query.join(Feature, isouter=True).where(Feature.copy_metrics["hook_type"].astext == hook_type)
    if has_numbers is not None:
        query = query.join(Feature, isouter=True).where(Feature.copy_metrics["has_numbers"].astext == str(has_numbers).lower())
    if min_runtime is not None:
        query = query.where(Ad.start_time.is_not(None)).where((func.coalesce(Ad.stop_time, datetime.utcnow()) - Ad.start_time) >= timedelta(days=min_runtime))
    if max_runtime is not None:
        query = query.where(Ad.start_time.is_not(None)).where((func.coalesce(Ad.stop_time, datetime.utcnow()) - Ad.start_time) <= timedelta(days=max_runtime))
    if min_reuse is not None:
        query = query.join(Score, isouter=True).where(Score.score_reuse >= min_reuse)
    if min_variants is not None:
        query = query.join(Score, isouter=True).where(Score.score_variants >= min_variants)
...
def get_ad(ad_id: str, db: Session = Depends(get_db)) -> dict:
    ...
    return {
        ...
        "score": {
            ...
            "saturation_index": ad.score.saturation_index if ad.score else 0,
            "explanation": ad.score.explanation if ad.score else {},
        },
        "hook": hook,
        "reuse_count": reuse_count,
        "variants_count": variants_count,
        "features": ad.features.copy_metrics if ad.features else {},
    }
