...
AdRadar Chat ist ein **regelbasierter** Suchassistent, der in Ads und Blueprints nach Treffern sucht und Quellen auflistet. Er hat **keinen** Zugriff auf externe Systeme und führt keine Aktionen aus. Erweiterungen (LLM, Tool-Execution) sind für spätere Phasen vorgesehen.

## Intelligence Layer (AIL)

Die Intelligence Layer liefert aggregierte Insights auf Basis von **Proxy-Signalen** (z. B. run length, Varianten, Friction):
- Parameter Strength (Feature-Lift)
- Pattern Combos
- Trend-Übersicht

Alle Metriken sind **Schätzwerte**, keine echten Lead-Zahlen.

## Advanced Intelligence (MVP Erweiterungen)

- **Creative Evolution**: Versionen pro Advertiser, Hook-Änderungen, Copy-Similarity (proxy).
- **Anti-Blueprints**: Kurzläufer (<3 Tage) und High-Friction als Negativbeispiele.

Alle Ergebnisse sind **proxy-basiert** und dienen der Orientierung.

## Outcome Feedback (MVP)

Eigene Ergebnisse können als Outcome-Feedback erfasst werden, um spätere Analytics zu kalibrieren.
